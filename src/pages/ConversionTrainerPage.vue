...
function checkPracticeAnswer(inputStr?: string): void {
  if (!practiceQuestion.value) return
  const expected = practiceQuestion.value.answer.toLowerCase()
  const answer = inputStr ?? conversionBoxRef.value?.getAnswer() ?? ''
  const got = normalizeAnswer(answer, conversionType.value)
