...
import { HomeIcon, FolderIcon, UserIcon, EnvelopeIcon } from '@heroicons/vue/24/solid'
...
<template>
  <nav class="fixed top-0 left-0 right-0 z-50 p-6 md:p-8">
    <div class="flex items-center justify-between max-w-7xl mx-auto">
      ...
      <div class="hidden md:flex items-center gap-2">
        ...
        >
          <span class="flex items-center gap-2">
            <component :is="item.icon" class="w-4 h-4 transition-transform duration-300 group-hover:rotate-12" />
            {{ item.name }}
          </span>
        </router-link>
      </div>
      ...
      >
        <Bars3Icon v-if="!isOpen" class="w-6 h-6 transition-transform duration-300" :class="{ 'rotate-90': isOpen }" />
